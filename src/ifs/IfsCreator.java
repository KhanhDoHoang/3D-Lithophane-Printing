...
public class IfsCreator {
	...
	public int[][] generateIfs(AffineTransform affineTrans){
		for(int i = 0 ; i < affineTrans.getWdith() * affineTrans.getHeight(); i++) {
			prob = Math.random();			
			int row = 0;
			...
			double u = affineTrans.getAffine()[row][A]*x+ affineTrans.getAffine()[row][B]*y + affineTrans.getAffine()[row][E];
			double v = affineTrans.getAffine()[row][C]*x + affineTrans.getAffine()[row][D]*y + affineTrans.getAffine()[row][F];
			x = u;
			y = v;
			xDisp =(int) (x * affineTrans.getScale() + affineTrans.getXoffset());
			yDisp =(int) (y * affineTrans.getScale() + affineTrans.getYoffset());			
